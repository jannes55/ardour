	...
	, _shared_with_ids (other->_shared_with_ids)
{
	...
	_end_space = cnt - (get_extent().first.distance (get_extent().second));
...
Playlist::add_region (boost::shared_ptr<Region> region, timepos_t const & position, float times, bool auto_partition)
{
	...
	if (times == 1 && auto_partition){
		RegionList thawlist;
		partition_internal (pos.decrement(), (pos + region->length()), true, thawlist);
	...
	*/
	...
	possibly_splice_unlocked (position, position.distance (pos + length), region);
}
...
Playlist::cut (timepos_t const & start, timecnt_t const & cnt, bool result_is_hidden)
{
	...
	{
		RegionWriteLock rlock (this);
		partition_internal (start, (start+cnt).decrement(), true, thawlist);
	}
...
Playlist::find_next_region (timepos_t const & start, RegionPoint point, int dir)
{
	...
	for (RegionList::iterator i = regions.begin(); i != regions.end(); ++i) {
		...
		case 1: /* forwards */
			...
			if (pos > start) {
				if ((distance = start.distance(pos)) < closest) {
					closest = distance;
					ret = r;
					end_iter = true;
				}
		...
		default: /* backwards */
			...
			if (pos < start) {
				if ((distance = pos.distance (start)) < closest) {
					closest = distance;
					ret = r;
				}
			} else {
	 ...
	 if (dir > 0) {
		 ...
		 for (RegionList::iterator i = regions.begin(); i != regions.end(); ++i) {
			 ...
			 if (r->position() > start) {
				 ...
				 distance = start.distance (r->position());
			 ...
			 if (r->last() > start) {
				 ...
				 distance = start.distance (r->last());
		 ...
		 for (RegionList::reverse_iterator i = regions.rbegin(); i != regions.rend(); ++i) {
			 ...
			 if (r->position() < start) {
				 ...
				 distance = r->last().distance (start);
			 ...
			 if (r->position() < start) {
				 ...
				 distance = r->position().distance (start);
		...
		for (i = regions.begin(); i != regions.end(); ++i) {
			...
			if ((*i)->position() >= start) {
				...
				if (forwards) {
					...
					if ((*i)->last_sample() > std::numeric_limits<timepos_t>::max().earlier (distance)) {
						new_pos = timepos_t::max().earlier ((*i)->length());
					} else {
