...
// apparently expand to ":: open"
	...
	if (_file_is_new && _length == 0 && writable() && !bwf_info_exists) {
		 ...
		 * in the original. */
		header_position_offset = _timeline_position.sample();
	}
...
SndFileSource::destructive_write_unlocked (Sample* data, samplecnt_t cnt)
{
	...
	if (_capture_start && _capture_end) {
		...
		*/
		...
		file_pos = capture_start_sample - _timeline_position.sample();
...
SndFileSource::set_header_timeline_position ()
{
	...
	_broadcast_info->set_time_reference (_timeline_position.sample());
...
{
	return _timeline_position.sample();
}
...
SndFileSource::crossfade (Sample* data, samplecnt_t cnt, int fade_in)
{
	...
	if (fade_position > _length) {
		...
		file_cnt = _length.samples() - fade_position;
...
void
SndFileSource::set_timeline_position (timepos_t const & pos)
{
