...
namespace Properties {
	...
	LIBARDOUR_API extern PBD::PropertyDescriptor<Temporal::LockStyle> position_lock_style;
...
public:
	...
	Temporal::LockStyle position_lock_style () const;
	void set_position_lock_style (Temporal::LockStyle ps);
	void recompute_position_from_lock_style ();

	void suspend_property_changes ();

	bool covers (timepos_t pos) const {
		return _position.val() <= pos && pos <= last();
	}
	...
	Temporal::OverlapType coverage (timepos_t const & start, timepos_t const & end) const {
		timepos_t const & a (_position.val());
		timepos_t const & b (last());
		timepos_t const & c (start);
		timepos_t const & d (end);
		//return Temporal::coverage (_position.val(), last(), start, end);
		return Temporal::coverage<timepos_t const &> (a, b, c, d);
	}
	...
	void trim_to (timepos_t const & position, timecnt_t const & length);

	/* fades are inherently audio in nature and we specify them in samples */
	virtual void fade_range (samplecnt_t, samplecnt_t) {}
	...
	void set_sync_position (timepos_t const &);
	...
	/* transients are all audio-based and therefore always use sample time
	 */
...
protected:
	...
	/* features (note onsets, transients, etc.) are all audio-based and
	 * thus always use sample time
	 */
