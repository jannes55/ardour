...
{
	const bool outside = (note->time() < _region->start()) || (note->time() >= _region->start() + _region->length());
...
MidiRegionView::add_canvas_patch_change (MidiModel::PatchChangePtr patch, const string& displaytext, bool /*active_channel*/)
{
	...
	if (patch_change->item().width() < _pixel_width) {
		...
		if (patch_time < 0 || patch_time >= _region->last()) {
			patch_change->hide();
		} else {
			patch_change->show();
		}
...
MidiRegionView::move_selection(Temporal::Beats const & dx_qn, double dy, double cumulative_dy)
{
	...
	Editor* editor = dynamic_cast<Editor*> (&trackview.editor());
	Temporal::TempoMap& tmap (editor->session()->tempo_map());
	...
	for (Selection::iterator i = _selection.begin(); i != _selection.end(); ++i) {
		...
		if (midi_view()->note_mode() == Sustained) {
			dx = editor->sample_to_pixel_unrounded (tmap.sample_at (note_time_qn + dx_qn))
...
MidiRegionView::move_copies (Temporal::Beats const & dx_qn, double dy, double cumulative_dy)
{
	...
	for (CopyDragEvents::iterator i = _copy_drag_events.begin(); i != _copy_drag_events.end(); ++i) {
		...
		if (midi_view()->note_mode() == Sustained) {
			Note* sus = dynamic_cast<Note*> (*i);
			double const len_dx = editor->sample_to_pixel_unrounded (tmap.sample_at (note_time_qn + dx_qn + n->note()->length()));
...
{
	PublicEditor& editor (trackview.editor());
	return snap_region_time_to_region_time (editor.pixel_to_sample (x), ensure_snap);
}
...
MidiRegionView::update_resizing (NoteBase* primary, bool at_front, double delta_x, bool relative, double snap_delta, bool with_snap)
{
	...
	for (std::vector<NoteResizeData *>::iterator i = _resize_data.begin(); i != _resize_data.end(); ++i) {
		...
		if (current_x < 0) {
			// This works even with snapping because RegionView::snap_region_time_to_region_time()
		...
		if (!cursor_set) {
			...
			samplecnt_t snap_delta_samps = trackview.editor().pixel_to_sample (snap_delta);
...
MidiRegionView::nudge_notes (bool forward, bool fine)
{
	...
	*/
	...
	if (!fine) {
		...
		timepos_t next_pos = ref_point;

		if (forward) {
			if (next_pos < std::numeric_limits<timepos_t>::max()) {
				next_pos.increment();
			}
		} else {
			if (next_pos == std::numeric_limits<timepos_t>::min()) {
				return;
			}
			next_pos.decrement ();
		}
...
bool
MidiRegionView::paste (Temporal::timepos_t const & pos, const ::Selection& selection, PasteContext& ctx)
{
	bool commit = false;
...
MidiRegionView::paste_internal (timepos_t const & pos, unsigned paste_count, float times, const MidiCutBuffer& mcb)
{
	...
	timepos_t region_end = (_region->position() + _region->length()).decrement();
...
MidiRegionView::update_ghost_note (double x, double y, uint32_t state)
{
	...
	const Temporal::Beats length = get_grid_beats (_region->position() + unsnapped_sample);
...
MidiRegionView::data_recorded (boost::weak_ptr<MidiSource> w)
{
	...
	for (MidiBuffer::iterator i = buf->begin(); i != buf->end(); ++i) {
		...
		*/
		...
		Temporal::Beats const time_beats = _region->absolute_time_to_source_beats (ev.time());
